...
export default function AdminDashboard() {
  ...
  const uniqueEmployees = ["All", ...Array.from(new Set(activities.map(a => a.employee_name).filter(Boolean)))];
  const filteredActivities = selectedEmployee === "All" ? activities : activities.filter(a => a.employee_name === selectedEmployee);
        ...
        <header className="flex flex-col md:flex-row justify-between items-start md:items-end gap-4 border-b border-white/5 pb-6">
          <div>
            ...
            <p className="text-slate-400 mt-2 text-lg font-medium tracking-wide">
              Welcome back, {adminName} • Live workforce activity
            </p>
          </div>
          <div className="flex items-center gap-3 flex-wrap justify-end">
            <div className="flex items-center gap-2 px-4 py-2 bg-slate-800/50 border border-white/10 rounded-full shadow-inner backdrop-blur-md">
              <span className="text-xs font-semibold text-slate-400 uppercase tracking-wider pl-1">Filter:</span>
              <select
                value={selectedEmployee}
                onChange={(e) => setSelectedEmployee(e.target.value)}
                className="bg-transparent text-sm font-semibold text-slate-200 outline-none cursor-pointer focus:ring-0 pr-1"
              >
            ...
            <div className="flex items-center gap-3 px-5 py-2.5 bg-indigo-500/10 border border-indigo-500/20 rounded-full shadow-inner backdrop-blur-md hidden sm:flex">
